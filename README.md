...
アップロード時に以下の情報を指定することで、より正確で詳細な議事録が生成されます：

- **会議の種類**: 定例会議、プロジェクト会議、ブレストなど
- **出席者**: 参加者の名前をカンマ区切りで指定
- **重点整理項目**: 決定事項、ネクストアクション、課題・リスク、議論のポイント
- **追加の指示**: 技術的な詳細を重視するなど、カスタム指示

### 🎨 議事録の構成（論点ベース構造）

生成される議事録は「論点（議題）」ごとに構造化されます：

- **要約（Summary）**: 会議全体の簡潔なまとめ
- **論点ごとの議事録（Agenda Items）**: 各論点について以下を記録
  - **論点（Issue）**: 「XXXはどうするか？」形式で議論テーマを記載
  - **議論内容（Discussion）**: 各発言者の発言を記録
  - **結論（Conclusion）**: 議論の結論を記載
  - **ネクスト論点（Next Issues）**: 派生した次の論点
  - **ネクストアクション（Next Actions）**: 担当者・アクション・期限
- **決定事項（Decisions）**: 全論点を通じた決定事項一覧（後方互換性）
- **ネクストアクション（Next Actions）**: 全論点を通じたアクション一覧（後方互換性）
