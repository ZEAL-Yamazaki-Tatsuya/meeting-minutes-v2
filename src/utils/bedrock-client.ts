...
1. **網羅性**: 決定事項とネクストアクションは、会議中に言及されたものをすべて記録してください。数が多くても省略しないでください。
2. **具体性**: 各項目は具体的かつ詳細に記述してください。曖昧な表現は避けてください。
3. **正確性**: 文字起こしテキストに忠実に、事実のみを記録してください。
4. **完全性**: 小さな決定やタスクも重要です。すべて記録してください。
5. **形式**: JSON形式のみを出力し、他の説明文は含めないでください。

決定事項やアクションアイテムが本当に存在しない場合のみ、空の配列を返してください。
担当者や期限が明示されていない場合は、そのフィールドを省略してください。
**タイムスタンプは必須項目です。文字起こしテキストの[HH:MM:SS]形式のタイムスタンプを必ず含めてください。タイムスタンプがない項目は記録しないでください。**`;
