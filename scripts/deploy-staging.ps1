# Staging環境へのデプロイスクリプト

Write-Host "🚀 Staging環境へのデプロイを開始します..." -ForegroundColor Cyan
...
try {
    ...
    if (Test-Path ".env.staging") {
        ...
        Write-Host "  ⚠️  .env.stagingファイルが見つかりません" -ForegroundColor Yellow
    }
    ...
    npx cdk deploy meeting-minutes-generator-storage-staging --require-approval never --context environment=staging
    if ($LASTEXITCODE -ne 0) { throw "Storage Stackのデプロイに失敗しました" }
    
    Write-Host "  → Auth Stack" -ForegroundColor Cyan
    npx cdk deploy meeting-minutes-generator-auth-staging --require-approval never --context environment=staging
    if ($LASTEXITCODE -ne 0) { throw "Auth Stackのデプロイに失敗しました" }
    
    Write-Host "  → Compute Stack" -ForegroundColor Cyan
    npx cdk deploy meeting-minutes-generator-compute-staging --require-approval never --context environment=staging
    ...
    $userPoolId = aws cloudformation describe-stacks `
        --stack-name meeting-minutes-generator-auth-staging `
...
"@
    ...
    npx cdk deploy meeting-minutes-generator-amplify-staging --require-approval never --context environment=staging
    ...
    # 12. デプロイ完了
    Write-Host "`n✅ Staging環境へのデプロイが完了しました！" -ForegroundColor Green
    Write-Host "`n📊 デプロイ情報:" -ForegroundColor Cyan
    Write-Host "  環境: Staging" -ForegroundColor White
