...
アップロード画面を改修し、従来の会議コンテキスト（オプション入力）に加えて、構造化された会議メタデータの入力フォームを追加しました。入力されたメタデータは議事録生成時のAIプロンプトに活用され、議事録一覧・詳細画面にも反映されます。

### 入力項目

| フィールド | 必須 | 説明 | バリデーション |
|-----------|------|------|--------------|
| 会議名（meetingTitle） | ○ | 会議の名称 | 空文字不可、最大100文字 |
| 開催日時（meetingDate） | ○ | 会議の開催日時 | ISO 8601形式 |
| 参加者（participants） | ○ | 参加者リスト | 1名以上、各名前最大50文字 |
| 論点（agenda） | × | 議論する論点リスト | 各項目最大200文字 |

### 論点（agenda）フィールドの活用

`agenda` フィールドは `Job` の `metadata` に `string[]` 型として保存されます。
...
- **論点が入力されている場合**: 入力された論点をベースに議事録を構造化するようAIに指示
- **論点が入力されていない場合**: AIが文字起こしから論点を自動抽出する（従来動作を維持）
- **どちらの場合も**: 同一の出力フォーマット（agendaItems構造：issue, discussion, conclusion, nextIssues, nextActions）で生成
- **論点数の制限**: 20件を超える場合は最初の20件に制限

### 動的入力欄

参加者と論点の入力欄は動的に追加・削除が可能です：

- 入力欄に文字を入力すると、空の入力欄が存在しなければ新しい入力欄が自動追加
- 各入力欄に削除ボタンを表示（入力欄が1つのみの場合を除く）
- 送信時に空の入力欄は自動的に除外

### 議事録一覧・詳細画面への反映

- **議事録一覧**: 会議名が設定されている場合は会議名を表示、`meetingDate` が存在する場合は開催日時を表示
- **議事録詳細**: ページタイトルとして会議名を表示、タイトル下に開催日時と参加者一覧を表示
- **後方互換性**: メタデータが存在しない過去の議事録は従来通りの表示を維持

### データフロー

```
フロントエンド（アップロードページ）
  ↓ metadata: { meetingTitle, meetingDate, participants, agenda }
アップロードAPI（upload-handler）
  ↓ DynamoDBのmetadataフィールドに保存
議事録生成Lambda（minutes-generator）
  ↓ metadata.agendaをプロンプトに含める
Amazon Bedrock（Claude 3.5 Sonnet v2）
  ↓ 論点ベースの構造化議事録を生成
```
